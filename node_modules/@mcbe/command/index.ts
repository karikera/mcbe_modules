...
export class Commander
{
    ...
    {
        const res = await this.command('scoreboard players list');
    ...
    async players_listOf(player:string):Promise<Scores>
    {
        ...
        const res = await this.command(`scoreboard players list "${player}"`);
        const lang = await Lang.onLoad;
        if (res.statusCode !== 0)
        {
    ...
    {
        const list = await this.command(`scoreboard objectives list`);
    ...
    /** teleport user */
    async tp(name:string, to:VectorXYZ):Promise<boolean>
    {
        const res = await this.command(`tp "${name}" ${coordForTp(to.x)} ${coordForTp(to.y)} ${coordForTp(to.z)}`);
        return res.statusCode === 0;
    }
    ...
    async killEntities(mask?:{type?:Identifier}):Promise<string[]>
    {
        ...
        const res:KillResult = await this.command(cmd);
        if (res.statusCode !== 0)
        {
            const lang = await Lang.onLoad;
...
/** teleport user */
export function tp(name:string, to:VectorXYZ):Promise<boolean>
{
    return commander.tp(name, to);
}
