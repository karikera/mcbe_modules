...
// If player out, it will fail
export async function replaceBlockBy(player:string, pos:VectorXYZ, from:Identifier, to:Identifier):Promise<boolean>
{
    const postext = pos.x+' '+pos.y+' '+pos.z;
    const res = await command(`execute "${player}" ~ ~ ~ detect ${postext} ${from} 0 execute "${player}" ~ ~ ~ setblock ${postext} ${to}`);
    return res.statusCode === 0;
}
