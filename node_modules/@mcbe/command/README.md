...
function setblockWithoutError(pos:VectorXYZ, block:Identifier, data?:number, option?:'destroy'|'keep'):Promise<boolean>;
...
/////////////////////////////////////////////////
// By Commands
// It's made for accessing nether
// but risky
...
// If player out, it will fail
function fillBy(player:string, from:VectorXYZ, to:VectorXYZ, type:Identifier, dataId?:number):Promise<boolean>;

// replace blocks with range, it can fill 32768(0x8000) blocks at one
// 'execute .. fill .. replace'
// If player out, it will fail
function fillReplaceBy(player:string, from:VectorXYZ, to:VectorXYZ, 
    type:Identifier, dataId:number, 
    typeFrom:Identifier, dataIdFrom:number):Promise<boolean>;

// test block at position with player
// it uses 'execute .. testforblock'
function testforblockBy(player:string, pos:VectorXYZ, type:Identifier):Promise<boolean>;

// set block at position with player
// it uses 'execute .. setblock'
// it will write log if it failed
function setBlockBy(player:string, to:VectorXYZ, type:Identifier, data?:number, option?:'destroy'|'keep'):Promise<boolean>;

// set block at position with player
// it's same with setBlockBy but no log
function setblockByWithoutError(player:string, to:VectorXYZ, type:Identifier, data?:number, option?:'destroy'|'keep');

// replace block at position with player
// it uses 'execute .. detect .. setblock'
// it will write log if it failed
function replaceBlockBy(user:string, pos:VectorXYZ, from:Identifier, to:Identifier):Promise<boolean>;

// make particle with user
function particleBy(user:string, id:string, pos:VectorXYZ):Promise<CommandResult>;
