    ...
    constructor(public now:number, private readonly getTime:(prev:number)=>number)
    {
        this.onUpdate = ()=>{
            ...
            for (;;)
            {
                ...
                node = <Timeout>axis.next;
                if (node === axis) return;
            }
        };

        ruacore.update.onBefore(this.onUpdate, nextTickOnUpdate);
    }
    
    insert(node:Timeout):void
    {
        const axis = this.axis;
        let find = <Timeout>axis.next;
        for (;;)
        {
