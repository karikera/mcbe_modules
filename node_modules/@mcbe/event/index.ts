...
import { systemShared } from "@mcbe/system_shared";
...
export function createBroadcast<T extends EventList>(map:T):BroadcastMapped<T>
{
    ...
    for (const key in map)
    {
        ...
        mapped[key] = (data:any)=>{
            const ev = systemShared.createEventData(mckey);
            if (!ev) throw Error('createEvent failed');
            ev.data = data;
            return systemShared.broadcastEvent(mckey, ev);
        };
    }
    return mapped;
}
...
export function createListener<T extends EventList>(map:T):ListenerList<T>
{
    ...
    for (const key in map)
    {
        ...
        mapped[key] = (cb:(ev:any)=>void)=>{
            systemShared.listenForEvent(mckey, ev=>{
                try
                {
                    cb(ev.data);
                }
    ...
    mapped_ex.all = function<KEY extends keyof T>(cb:(ev:EventObject<T, KEY>)=>void){
        ...
        {
            systemShared.listenForEvent(type, ev=>{
...
systemShared.initialize = ()=>sharedEvents.initialize.fire();
systemShared.shutdown = ()=>sharedEvents.shutdown.fire();
systemShared.update = ()=>sharedEvents.update.fire();
Object.freeze(systemShared);
