...
## About this
It's `Map<string, Map<string, number>>` style storage with using scoreboard  
It will put everything to internal variable  
It uses prefix to avoid duplication  
  
So, It can write/read without any delay  
It will wait 1sec before puting to scoreboard, and flush all after 1 sec  
So, **It needs timer supports**
...
Store.onLoad.then(loaded=>{ // wait loading, It will read all of scoreboard
    loaded.scoreboard; // string array, remained player names
        
    const chest1 = store1.get('x_y_z'); // player name of scoreboard is chest_x_y_z
    const chest2 = store1.get('1_2_3'); // player name of scoreboard is chest_1_2_3
    // player name length is no limited, I tested over 4*1024*1024 characters

    const item1count = chest1.get('item1') || 0; // get 'item1' value or 0, 
